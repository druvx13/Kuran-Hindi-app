...
public final class zzcnc {
    public static ArrayList<zztf.zzo.zza> zza(SQLiteDatabase sQLiteDatabase) {
        ArrayList<zztf.zzo.zza> arrayList = new ArrayList<>();
        Cursor query = sQLiteDatabase.query("offline_signal_contents", new String[]{"serialized_proto_data"}, null, null, null, null, null);
        while (query.moveToNext()) {
            try {
                arrayList.add(zztf.zzo.zza.zzg(query.getBlob(query.getColumnIndexOrThrow("serialized_proto_data"))));
            } catch (zzeco e) {
                zzawr.zzfa("Unable to deserialize proto from offline signals database:");
                zzawr.zzfa(e.getMessage());
            }
        }
        query.close();
        return arrayList;
