...
public class VersesBookmarkDetailsListAdapter extends BaseAdapter {
    ...
    private ArrayList<VersesBookmarkQuranDetails> quranDetails;
    ...
    public VersesBookmarkDetailsListAdapter(Context context, String str, ArrayList<VersesBookmarkQuranDetails> arrayList) {
    ...
    public static void CustomDialog(final Context context, final String str, int i) {
        int abs;
        float abs2;
        ...
        double parseDouble = Double.parseDouble(Utils.getString((Activity) context, "device_height"));
        ...
        int i2 = (int) abs2;
        verse_id = DatabaseAccessor.getV_B_VersesIdById(str.split("_")[1]).getVerse_id();
        String str2 = str.split("_")[0];
        if (i == 0 && !str2.equals("9")) {
            attributes.y = VersesBookmarkViewPagerFragment.new_top + i2;
        } else {
            attributes.y = VersesBookmarkViewPagerFragment.new_top + abs;
        }
        ((RelativeLayout) dialog.findViewById(R.id.relCopy)).setOnClickListener(new View.OnClickListener() { // from class: com.muslim.Bookmark.Verses.VersesBookmarkDetailsListAdapter.1
            ...
            public void onClick(View view) {
                ...
                String str3 = context.getResources().getString(R.string.sura) + " " + suraById.getSuraName() + " (" + suraById.getSura_name_ar() + ")" + context.getResources().getString(R.string.verse) + " " + Utils.getBanglaNumber(versesByVersesId.getVerse_id()) + "\n\n" + versesByVersesId.getModified_content() + "\n\n" + DatabaseAccessor.getVersesByVersesId(str.split("_")[1]).getContent();
                ClipboardManager clipboardManager = (ClipboardManager) context.getSystemService("clipboard");
                ClipData newPlainText = ClipData.newPlainText(context.getResources().getString(R.string.sura) + " " + suraById.getSuraName() + "(" + suraById.getSura_name_ar() + ")" + context.getResources().getString(R.string.tvVerse) + " " + Utils.getBanglaNumber(versesByVersesId.getVerse_id()), str3 + ("\n\nPlaystore Link: \nhttps://play.google.com/store/apps/details?id=" + context.getPackageName()));
        ...
        ((ImageView) dialog.findViewById(R.id.bookmark)).setImageResource(R.drawable.ic_star_bookmark);
        final String str3 = str.split("_")[0];
        final String str4 = str.split("_")[1];
        ((RelativeLayout) dialog.findViewById(R.id.relBookmark)).setOnClickListener(new View.OnClickListener() { // from class: com.muslim.Bookmark.Verses.VersesBookmarkDetailsListAdapter.2
            @Override // android.view.View.OnClickListener
            public void onClick(View view) {
                VersesBookmarkDetailsListAdapter.dialog.dismiss();
                Context context2 = context;
                Utils.putString(context2, "bookmark_remove" + str, "");
                Context context3 = context;
                Utils.putString(context3, "bookmark_verse" + str4, "");
                Constants.mi = 1;
                VersesBookmarkActivity.getInstance().getSupportFragmentManager().popBackStack();
                VersesBookmarkActivity.getInstance().prepareListDataBookmark();
                Utils.setBanglaText((Activity) context, VersesBookmarkActivity.mTitle, context.getResources().getString(R.string.my_saved_ayats));
                Toast.makeText(context, "Bookmark Removed", 0).show();
            }
        });
        ((RelativeLayout) dialog.findViewById(R.id.relShare)).setOnClickListener(new View.OnClickListener() { // from class: com.muslim.Bookmark.Verses.-$$Lambda$VersesBookmarkDetailsListAdapter$31O4J0wQDl2IA3Ksjr_Adwl5FuE
            @Override // android.view.View.OnClickListener
            public final void onClick(View view) {
                VersesBookmarkDetailsListAdapter.lambda$CustomDialog$0(str, context, view);
            }
        });
        ((RelativeLayout) dialog.findViewById(R.id.relPlay)).setOnClickListener(new View.OnClickListener() { // from class: com.muslim.Bookmark.Verses.-$$Lambda$VersesBookmarkDetailsListAdapter$j0p9sgDHa-v5HPND2CMDnqHN2Ec
            @Override // android.view.View.OnClickListener
            public final void onClick(View view) {
                VersesBookmarkDetailsListAdapter.lambda$CustomDialog$1(str3, context, view);
            }
        });
        dialog.show();
    }
    ...
    /* JADX INFO: Access modifiers changed from: package-private */
    public static /* synthetic */ void lambda$CustomDialog$1(String str, Context context, View view) {
        dialog.dismiss();
    ...
    private static void playAudio(final Activity activity, String str, final String str2, String str3) {
        ...
        if (!new File(Utils.getRoot().getAbsolutePath() + "/QuranIDEA/Audio/" + Utils.getString(activity, Constants.KEY_RECEITER) + "/" + audioFolder + "/" + audioFolder + audioFile + ".mp3/").exists()) {
            ...
            }).setNegativeButton(activity.getResources().getString(R.string.cancel), new DialogInterface.OnClickListener() { // from class: com.muslim.Bookmark.Verses.VersesBookmarkDetailsListAdapter.3
                @Override // android.content.DialogInterface.OnClickListener
                public void onClick(DialogInterface dialogInterface, int i) {
    ...
    public static void singlePlay(Activity activity) {
        ...
        mp = create;
        create.start();
        mp.setOnCompletionListener(new MediaPlayer.OnCompletionListener() { // from class: com.muslim.Bookmark.Verses.VersesBookmarkDetailsListAdapter.5
    ...
    public View getView(int i, View view, ViewGroup viewGroup) {
        ...
        if (Utils.getString((Activity) this.context, "normal").equals("")) {
            this.holder.relBismillah.setBackgroundColor(this.context.getResources().getColor(R.color.relBismillah_attract));
            if (i % 2 == 0) {
            ...
            this.holder.relBismillah.setBackgroundColor(this.context.getResources().getColor(R.color.relBismillah_normal));
            if (i % 2 == 0) {
        ...
        int parseInt2 = Integer.parseInt(this.quranDetails.get(i).getId());
        int parseInt3 = Integer.parseInt(DatabaseAccessor.getBookmarkSuraIdByVersesId(parseInt2 + "").getSura_id());
        if (parseInt3 == 1) {
            parseInt = 0;
        } else {
            ...
            sb.append("");
            parseInt = Integer.parseInt(DatabaseAccessor.getBookmarkVersesIdBySuraId(sb.toString()).getId());
        }
        int i2 = parseInt2 - parseInt;
        Log.d("DREG", parseInt2 + "_" + parseInt3 + "_" + parseInt);
        if (Utils.getString((Activity) this.context, Constants.KEY_TRANSLATOR).equals(Constants.TRANS_INTERNATIONAL)) {
            str = i2 + ". " + this.quranDetails.get(i).getContent();
        } else {
            str = Utils.getBanglaNumber(i2) + ". " + this.quranDetails.get(i).getContent();
        }
        Utils.setTransliterationText((Activity) this.context, this.holder.tvTransliteration, i2 + ". " + this.quranDetails.get(i).getTrans().trim());
        Utils.setBanglaText((Activity) this.context, this.holder.tvSource, str.trim());
        if (Build.VERSION.SDK_INT >= 23) {
            Utils.setArabicText((Activity) this.context, this.holder.tvTitle, modified_content, i2);
        } else if (!Utils.getString((Activity) this.context, Constants.KEY_ARABIC_FONT).equals(Constants.FONT_NOOR_E_HIDAYAT) && !Utils.getString((Activity) this.context, Constants.KEY_ARABIC_FONT).equals(Constants.FONT_NOOR_E_HUDA) && (this.sura_id.equals("112") || this.sura_id.equals("113") || this.sura_id.equals("114"))) {
            Utils.setArabicText((Activity) this.context, this.holder.tvTitle, Grammer.arabic(this.context, modified_content).trim(), true);
        } else {
            Utils.setArabicText((Activity) this.context, this.holder.tvTitle, modified_content, false);
        }
        return view;
    }
