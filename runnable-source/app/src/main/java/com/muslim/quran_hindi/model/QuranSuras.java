package com.muslim.quran_hindi.model;

import java.util.List;

public class QuranSuras {
    private String audioLink;
    private String page;
    private List<QuranDetails> quranDetails;
    private String sura_id;
    private String sura_meaning;
    private String sura_name;
    private String sura_name_ar;
    private String sura_reverse_id;
    private String versesId;

    public String getSura_id() {
        return this.sura_id;
    }

    public void setSura_id(String str) {
        this.sura_id = str;
    }

    public String getVersesId() {
        return this.versesId;
    }

    public void setVersesId(String str) {
        this.versesId = str;
    }

    public String getSura_reverse_id() {
        return this.sura_reverse_id;
    }

    public void setSura_reverse_id(String str) {
        this.sura_reverse_id = str;
    }

    public String getSura_name_ar() {
        return this.sura_name_ar;
    }

    public void setSura_name_ar(String str) {
        this.sura_name_ar = str;
    }

    public String getSuraName() {
        return this.sura_name;
    }

    public void setSuraName(String str) {
        this.sura_name = str;
    }

    public String getSuraMeaning() {
        return this.sura_meaning;
    }

    public void setSuraMeaning(String str) {
        this.sura_meaning = str;
    }

    public String getAudioLink() {
        return this.audioLink;
    }

    public void setAudioLink(String str) {
        this.audioLink = str;
    }

    public String getPage() {
        return this.page;
    }

    public void setPage(String str) {
        this.page = str;
    }

    public List<QuranDetails> getQuranDetails() {
        return this.quranDetails;
    }

    public void setQuranDetails(List<QuranDetails> list) {
        this.quranDetails = list;
    }
}
